...
from geppo.utils import get_latest_run_id
...
def run(inputs_dict):
    ...
    alg_kwargs['save_dir'] = setup_kwargs['save_dir']
    ...
    eval_env = init_env(**env_kwargs)

    init_seeds(ac_seed)
    actor = init_actor(env,**actor_kwargs)
    critic = init_critic(env,**critic_kwargs)
    runner = Runner(**runner_kwargs)
    alg = init_alg(sim_seed,env,eval_env,actor,critic,runner,ac_kwargs,**alg_kwargs)
...
def main():
    ...
    args.save_dir = f"{args.results_dir}/{args.env_name}/geppo/{args.results_subdir}"
    ...
    inputs_dict['setup_kwargs']['save_dir'] = args.save_dir
